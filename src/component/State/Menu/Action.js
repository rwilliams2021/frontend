...
export const createMenuItem = ({ menu, jwt }) => async (dispatch) => {
    ...
    try {
        const { data } = await api.post(`${API_URL}/admin/food/create/`, menu, {
            headers: {
...
export const updateMenuItemsAvailability = ({ foodId, jwt }) => async (dispatch) => {
    ...
    try {
        const { data } = await api.put(`${API_URL}/admin/food/${foodId}`, {}, {
            headers: {
                Authorization: `Bearer ${jwt}`
            }
        })
...
export const deleteMenuItem = ({ foodId, jwt }) => async (dispatch) => {
    ...
    try {
        const { data } = await api.delete(`${API_URL}/admin/food/${foodId}`, {
            headers: {
                Authorization: `Bearer ${jwt}`
            }
        })
...
export const getMenuItemsByRestaurantId = (reqData) => async (dispatch) => {
    ...
    try {
        const { data } = await api.get(`${API_URL}/food/restaurant/${reqData.restaurantId}?vegetarian=${reqData.vegetarian}
...
export const searchMenutItems = ({ keyword, jwt }) => async (dispatch) => {
    ...
    try {
        const { data } = await api.get(`${API_URL}/food/search?name=${keyword}`, {
            headers: {
                Authorization: `Bearer ${jwt}`
            }
        })
